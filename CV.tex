...
\begin{cvevent}[2025]
    \cvname{go-mem}
    \cvdescription{Personal Project}
    In order to teach myself some things about the Go programming language as well as AI tools, I vibe-coded \texttt{go-mem}, a console mode game to assist in memorizing texts.  The project may be viewed and downloaded at https://github.com/ArkieCoder/go-mem.
    \vspace*{5mm}
\end{cvevent}
...
\begin{cvevent}[2025]
    ...
    Implemented a comprehensive labeling scheme in our Kubernetes cluster to comply with SPS Commerce's post-acquisition requirements, along with a tagging regime required by SPS with regard to our AWS infrastructure.  Additionally, architected the solution to switch Kubernetes secrets from HashiCorp Vault secrets where Vault ran within our clusters, to using AWS Secrets Manager where the secrets existed in a different AWS account.  This required a lot of IAM wrangling and communication with teams in order to coordinate changes.    
    \vspace*{5mm}
\end{cvevent}
